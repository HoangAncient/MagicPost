...
import { LineData } from "../../assets/data/LineData";
...
export default function App() {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axiosInstance.get("/api/auth/all_packages?" + time);
  
      const transformedPieData = response.data.pieData.map(({ name, quantity }) => ({
        ...
        datasets: [
          {
            ...
            backgroundColor: chartPieData.datasets[0].backgroundColor.slice(0, transformedPieData.length), // Using existing colors or add more as needed
            borderColor: chartPieData.datasets[0].borderColor.slice(0, transformedPieData.length), // Using existing colors or add more as needed
            borderWidth: 0,
          },
        ],
      });
    ...
    <div className="Home">
      ...
      <div className="homeContainer">
        ...
        <div className="widgets">
          <Widget type="totalGoodsReceived" role="managerExchange"/>
          <Widget type="totalGoodsSent" role="managerExchange" />
          <Widget type="employee" role="managerExchange" />
        </div>
